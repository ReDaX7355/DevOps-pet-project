...
В Github репозитории были созданы пайплайны, для тестирования приложения, создания образов (Forntend и Backend) и деплоя на сервере.
...
С помощью Ansible, была создана конфигурация, для настройки окружения и деплоя приложения с мониторингом на отдельные сервера. Она используется при первом развертывании на удаленных серверах.
...
Далее созданы конфиги `inventory.yml` и `ansible.cfg` для настройки хостов, где будут разворачиваться приложение с мониторингом.
...
# 2. Записать публиные IP созданных серверов в inventory.yml
...
- автоматизированная настройка окружения и деплой на удаленные сервера с помощью Ansible

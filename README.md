...
C помощью Terraform, для веб-приложения была создана конфигурация для автоматического развертывания облачной инфраструктуры на основе провайдера Selectel. 
Она имитирует prodution-сценарий, где создается 2 сервера в облаке, отдельно для самого приложения и для мониторинга.

### Требования
- Terraform 1.15+
- Аккаунт Selectel с пополненным балансом
- SSH ключ загруженный в ЛК Selectel
...
./terraform/
  ...
  - network.tf # Создание сетевой инфраструктуры (роутер, внтуренняя сеть, открытие необходимых портов)
  ...
  - outputs.tf # Вывод IP адресов серверов и строки для подключения по SSH

```

Также в конфигурации (./.terraformrc)  настроено зеркало, для установки модулей провайдера, т.к. официальное RHCP не работает.
